...
public class RoleService : IRoleService
{
    ...
    {
        var user = await _userRepository.GetAll().Include(r => r.Roles).FirstOrDefaultAsync(u => u.Login == dto.Login);
        if (user == null)
        {
            return new BaseResult<UserRoleDto>()
            {
        ...
        var role = await _roleRepository.GetAll().FirstOrDefaultAsync(r => r.Id == dto.RoleId);
        if (role == null)
        {
            return new BaseResult<UserRoleDto>()
            {
        ...
        var userRole = await _userRoleRepository.GetAll().Where(x => x.RoleId == role.Id).FirstOrDefaultAsync(x => x.UserId == user.Id);
    ...
    public async Task<BaseResult<UserRoleDto>> UpdateRoleForUserAsync(UpdateUserRoleDto dto)
    {
        ...
        var role = await _roleRepository.GetAll().FirstOrDefaultAsync(r => r.Id == dto.FromRoleId);
        if (role == null)
        {
            return new BaseResult<UserRoleDto>()
            {
